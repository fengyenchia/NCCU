...
function draw() {
  ...
  // let countX = 17;
  // let countY = 25;
  // let sizeX = (width - 2 * padding) / (countX - 1);
  // let sizeY = (height - 2 * padding) / (countY - 1);
  // stroke(255);
  // push();
  ...
  textFont(font2);
  strokeWeight(1.5);
  let points1 = font2.textToPoints('N', padding * 2, height / 2 + padding * 1.8, 180, { sampleFactor: 0.04 });
  let points2 = font2.textToPoints('{', padding * 5.7, height / 2 + padding * 1.8, 180, { sampleFactor: 0.04 });
  // let pointPositionX = [];
  // let pointPositionY = [];
  // for (let p of points) {
  ...
  let _textBox = font2.textBounds('}', width - padding * 2, height / 2 + padding * 1.8, 180);
  let points3 = font2.textToPoints('}', width - padding * 5.7 - _textBox.w, height / 2 + padding * 1.8, 180, { sampleFactor: 0.04 });
  let UtextBox = font2.textBounds('U', width - padding * 2, height / 2 + padding * 1.8, 220);
  let points4 = font2.textToPoints('U', width - padding * 2 - UtextBox.w, height / 2 + padding * 1.8, 180, { sampleFactor: 0.04 });
...
// 主辦單位：政大區塊鏈應用課程期末成果展籌備小組、c2x3
  ...
  strokeWeight(0);
  textSize(20);
  text('主辦單位 | 政大區塊鏈應用課程期末成果展籌備小組、超級浪、c2x3', width / 2, height - padding * 2.5);
